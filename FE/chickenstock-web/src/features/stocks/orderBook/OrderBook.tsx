...
const OrderBook = () => {
  ...
  return ( 
    <div className="flex h-full flex-col rounded-lg border border-gray-200 bg-white shadow-sm overflow-hidden"> 
      ...
      <footer className="border-t border-gray-200 px-6 py-2 text-xs text-gray-500 flex justify-between">
        <span>총 매수잔량: {mockOrderBookData.reduce((sum, item) => sum + item.bidVolume, 0).toLocaleString()}</span>
        <span>총 매도잔량: {mockOrderBookData.reduce((sum, item) => sum + item.askVolume, 0).toLocaleString()}</span>
      </footer>
    </div> 
  ); 
}; 
