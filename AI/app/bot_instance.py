...
class BotInstance:
    ...
    def __init__(self, email: str, strategy: TradingStrategy):
        """봇 인스턴스 초기화"""
        self.email = email
    ...
    async def initialize(self, password: str):
        ...
        try:
            ...
            # 키움 API 초기화
            self.kiwoom_api = KiwoomAPI(self.auth_client)
            
            # 백엔드 클라이언트 초기화
            self.backend_client = BackendClient(self.auth_client)
            ...
            # 백엔드 클라이언트에 트레이딩 모델 연결
            self.backend_client.set_trading_model(self.trading_model)
    ...
    async def start(self):
        ...
        try:
            ...
            # 필터링된 종목 리스트 가져오기
            initial_filtered_symbols = await self.kiwoom_api.get_filtered_symbols(600, 200)
            ...
            # 거래 처리 루프 시작
            self.trading_loop_task = asyncio.create_task(self.run_trading_loop())
    ...
    async def run_trading_loop(self):
        """거래 처리 루프"""
        logger.info(f"봇 {self.email} 거래 처리 루프 시작")
        ...
        while self.is_running:
            try:
                # 백엔드 서버 인증 상태 확인
                if not self.auth_client or not self.auth_client.is_authenticated:
                    logger.error(f"봇 {self.email} 백엔드 서버 인증되지 않았습니다. 거래를 건너뜁니다.")
                    ...
                    for decision in decisions:
                        try:
                            ...
                            if action.lower() == "buy":
                                ...
                                if result:
                                    logger.info(f"봇 {self.email} 매수 요청 성공: {symbol} {quantity}주")
                                else:
                                    logger.error(f"봇 {self.email} 매수 요청 실패: {symbol} {quantity}주")
                            
                            elif action.lower() == "sell":
                                result = await self.backend_client.request_sell(symbol, quantity, price)
                                if result:
                                    logger.info(f"봇 {self.email} 매도 요청 성공: {symbol} {quantity}주")
                                else:
                                    logger.error(f"봇 {self.email} 매도 요청 실패: {symbol} {quantity}주")
                            
                        except Exception as e:
                            logger.error(f"봇 {self.email} 거래 요청 전송 중 오류: {str(e)}")
                ...
                logger.info(f"봇 {self.email} 거래 처리 루프 취소됨")
                break
            except Exception as e:
                logger.error(f"봇 {self.email} 거래 처리 루프 오류: {str(e)}")
                await asyncio.sleep(30)  # 오류 시 30초 후 재시도
    
    async def run_scheduler(self):
        """스케줄러 (정기 데이터 갱신)"""
        logger.info(f"봇 {self.email} 스케줄러 시작")
        ...
        while self.is_running:
            try:
                ...
                logger.info(f"봇 {self.email} 다음 차트 데이터 수집 및 지표 계산까지 {wait_seconds:.2f}초 남음")
                
                # 최대 12시간까지만 대기
                await asyncio.sleep(min(wait_seconds, 43200))
                
                # 서비스가 여전히 실행 중이면 작업 수행
                if self.is_running:
                    logger.info(f"봇 {self.email} 정기 스케줄에 따른 차트 데이터 수집 및 지표 계산 시작")
                    
                    # 전략에 따른 차트 데이터 수집 및 지표 계산
                    if self.strategy == TradingStrategy.ENVELOPE:
                        # Envelope 지표 갱신
                        filtered_stockcode_list = self.kiwoom_api.stock_cache.filtered_stockcode_list
                        await self.kiwoom_api.initialize_chart_data(filtered_stockcode_list, period=120)
                        self.kiwoom_api.stock_cache.calculate_envelope_indicators()
                    
                    elif self.strategy == TradingStrategy.BOLLINGER:
                        # 볼린저 밴드 지표 갱신 - StockCache 통합 메서드 사용
                        filtered_stockcode_list = self.kiwoom_api.stock_cache.filtered_stockcode_list
                        await self.kiwoom_api.initialize_chart_data(filtered_stockcode_list, period=60)
                        self.kiwoom_api.stock_cache.calculate_bollinger_bands()
                    ...
                    logger.info(f"봇 {self.email} 정기 스케줄에 따른 차트 데이터 수집 및 지표 계산 완료")
                
            except asyncio.CancelledError:
                logger.info(f"봇 {self.email} 스케줄러 취소됨")
                break
            except Exception as e:
                logger.error(f"봇 {self.email} 스케줄러 오류: {str(e)}")
    ...
    def get_status(self) -> Dict:
        ...
        return {
            ...
            "account_info": {
                "cash_balance": self.kiwoom_api.account_info["cash_balance"] if self.kiwoom_api else 0,
                "positions_count": len(self.kiwoom_api.account_info["positions"]) if self.kiwoom_api else 0,
                "total_asset_value": self.kiwoom_api.account_info["total_asset_value"] if self.kiwoom_api else 0
