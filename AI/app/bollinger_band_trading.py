...
class BollingerBandTradingModel:
    ...
    async def start(self):
        ...
        # 볼린저 밴드 지표 계산 (캐시 활용)
        await self.calculate_indicator_cache()
    ...
    async def calculate_indicator_cache(self):
        """지표 캐시 계산 (StockCache 활용)"""
        logger.info("볼린저 밴드 지표 캐시 계산 시작")
        
        # StockCache를 사용하여 볼린저 밴드 지표 계산
        if hasattr(self.kiwoom_api.stock_cache, 'calculate_bollinger_bands'):
            # 새 버전의 StockCache 사용 (계산 함수가 있는 경우)
            success_count = self.kiwoom_api.stock_cache.calculate_bollinger_bands()
            logger.info(f"볼린저 밴드 지표 캐시 계산 완료: {success_count}개 종목")
        elif hasattr(self.kiwoom_api.stock_cache, 'calculate_all_indicators'):
            # 통합 계산 메서드가 있는 경우
            _, bollinger_success = self.kiwoom_api.stock_cache.calculate_all_indicators()
            logger.info(f"볼린저 밴드 지표 통합 캐시 계산 완료: {bollinger_success}개 종목")
        else:
            logger.warning("볼린저 밴드 지표 계산 메서드를 찾을 수 없음 - 지표 계산 건너뜀")
